...
//     const withProgress = lessons.map(l => ({ ...l.toObject(), isCompleted: completedIds.includes(l._id.toString()) }));
...
//     }
//     res.json({ success: true, message: "Dars tugallandi! 🎉", completedCount: req.user.completedLessons.length });
//   } catch (err) { next(err); }
// });
...
//       return res.json({ success: true, message: "Ma'lumotlar allaqachon mavjud" });
...
//       { title: "Anemiya",                              description: "Temir tanqisligi, belgilari, sabablari va davolash.",                icon: "🩸", color: "#dc2626", bgColor: "#fef2f2", isPro: true,  order: 5 },
//     ]);
//     const lessons = [];
//     courses.forEach(c => {
...
//   res.json({ success: true, service: "Porla Backend", version: "1.0.0",
...
async function autoSeed() {
  ...
  try {
    const cList = await Course.insertMany([
      { title: "Ayollar reproduktiv tizimi",           description: "Reproduktiv organlar, ularning vazifalari va asosiy tushunchalar.",    icon: "🩺", color: "#3b7de8", bgColor: "#eff6ff", isPro: false, order: 1 },
      { title: "Hayz sikli",                           description: "Normal tsikl, uning fazalari, ta'sir etuvchi omillar va kuzatish.",     icon: "🌸", color: "#d64f6e", bgColor: "#fde8ec", isPro: false, order: 2 },
      { title: "Ayol gormonlari salomatlikdagi o'rni", description: "Gormonlar, ularning muvozanati va sog'liqqa ta'siri haqida kurs.",      icon: "⚗️", color: "#8657d6", bgColor: "#f3f0ff", isPro: true,  order: 3 },
